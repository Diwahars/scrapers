...
class MySpider(CrawlSpider):
  ...
  rules = (Rule (SgmlLinkExtractor(restrict_xpaths=('//td[@class="tdMenu"][3]/div/div',))
  , callback="parse_giant", follow= True),)
  
  csvfile = None
  printHeader = True
  def to_csv(self, item):
    if self.printHeader:
      self.csvfile = open('starkennbikes.csv','w')
    if self.csvfile:
  ...
  def parse_giant(self,response):
    
    sel = Selector(response)
    item = BigCItem()
    url = sel.xpath("//td[@align='center']/a/@href").extract()
    item["Brand_Name"] = "GIANT"
    for x in url:
...
##      
  ...
  def parse_items(self, response):
      ...
      cat = sel.xpath("//span[@id='ctl00_ContentPlaceHolder1_lblPagePath']/a/text()").extract()[0]
      item["Category"] = cat
      if cat == 'ON-ROAD':
        item["Category"] = "Run & Cycle/Cycling Bikes/Road Bikes"
      elif cat == 'OFF-ROAD':
        item["Category"] = "Run & Cycle/Cycling Bikes/Mountain Bikes"
      elif cat == 'X-ROAD':
        item["Category"] = "Run & Cycle/Cycling Bikes/Road Bikes"
