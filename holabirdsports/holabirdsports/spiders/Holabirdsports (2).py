...
##      item = BigCItem()   
      ...
      item["Brand_Name"]  = sel.xpath("//script[@type='text/javascript'][contains(text(),'Brand :')]").extract()[0].split("Brand : \'")[-1].split("\'")[0]
      ...
      if response.xpath("//span[@class='msrp_price']/text()").extract():
        ...
        if category =='Shoes':
          mrp = round(((float(mrp)*65 + 2100)*114.5/100 + float(mrp)*65*23/100),0)
          sp = round(((float(sp)*65 + 2100)*114.5/100 + float(sp)*65*26/100),1)
        else:           
           mrp = round(((float(mrp)*65)*105.5/100 + float(mrp)*65*18/100),0)
           sp = round(((float(sp)*65)*105.5/100 + float(sp)*65*18/100),1)
      else:
      ...
      if category =="Shoes":
        ...
        if 'Squash' in x:          
          if "ASICS" in item["Brand_Name"] or "adidas" in item["Brand_Name"]:
            sortorder = -350            
          elif "Salming" in item["Brand_Name"]:
            sortorder = -330
          else:
            sortorder = -290
          if "Women" in pname:
      ...
      sku = (sel.xpath("//script[@type='text/javascript'][contains(text(),'Brand :')]").extract()[0].split("prodid: \'")[-1].split("\'")[0].strip()
             +"HLABRD")
      description = sel.xpath("//div[@id='panel_description']/p").extract()
      if description:
        description = re.sub(r'<a(.*)>','',sel.xpath("//div[@id='panel_description']/p").extract()[0])
      else:
        description = ''
      ...
      tup = ("Product",'',pname+"*",item["Brand_Name"],
           mrp,mrp,sp,          #price
