...
class vlookup(object):
    ...
    def __init__(self):        
        ...
        # self.input_file = 'HolabirdSports.csv'
        # self.binpicking_number = 'HOLABIRDSPORTS'
        ...
        for row in master_dict:
            if row['Bin Picking Number'].lower().strip() == self.binpicking_number.lower().strip():
                self.product_id_dict[row['Product Code/SKU']] = row['Product ID']
        self.header = True
        master_file.close()
    ...
    def vlookup(self):
        ...
        for new_row in csv_dict:
            if new_row['Product Code/SKU'] in self.product_id_dict:                
                new_row['Product ID'] = self.product_id_dict[new_row['Product Code/SKU']]
