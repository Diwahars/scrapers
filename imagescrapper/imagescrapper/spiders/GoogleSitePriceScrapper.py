...
class MySpider(CrawlSpider):
  dont_filter=True
  name = "google"
  allowed_domains = ["snapdeal.com", "amazon.in", "amazon.com", "flipkart.com","google.com","google.co.in"]

  start_urls = [
    "https://www.google.com/#q=christopher+nolan"
    #"http://www.liveyoursport.com/products/fuel-elliptical-4-0"
##    url.strip() for url in productname_list
    ]

  def parse(self,response):
    sel = Selector(response)
    x = sel.xpath("//a/@href").extract()
    print x 
...
##      #headers
##      if self.printHeader: 
##        strWrite +='LYS Product Name,LYS URL,LYS Product Code, LYS MRP,LYS SP,LYS Stock,Category,'
...
##      
##    request=Request("http://www.snapdeal.com/search?noOfResults=20&keyword="+item ["LYSProduct_Name"].replace("Squash Racket",""), self.snapdeal)    
...
##    item = response.meta['item']
##    sel = Selector(response)
##    result =  response.xpath("//a[@class='a-link-normal s-access-detail-page  a-text-normal']").extract()
##    
##    if result:
##      
