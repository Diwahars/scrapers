      ...
      for sizes in item['size']:        
          if item['id1'] !="" :
            for colors in item["color"]:
              if item['Category'] == "Team Sports/Hockey/Hockey Shoes;Shoes/Men's Shoes/Hockey Shoes and Spikes":
                strWrite += 'SKU,[S]Size=UK ' + sizes.split("Shoe")[-1].replace("Size","") + '; [S]' + item['id1'].extract()[0].replace("\n","").replace("\r","").replace("  ","") + "=" + colors + ',,,,,,' + item ["Product_Code"]+ "UK" + sizes.split("Size")[-1].replace('"',"") + colors + 'HKD,,,,,,,,,,,,,,,,\n'

              elif item['Category'] == "Team Sports/Hockey/Hockey Shoes;Shoes/Women's Shoes/Hockey Shoes and Spikes":
                strWrite += 'SKU,[S]Size=UK ' + sizes.split("Shoe")[-1].replace("Size","") + '; [S]' + item['id1'].extract()[0].replace("\n","").replace("\r","").replace("  ","") + "=" + colors + ',,,,,,' + item ["Product_Code"]+ "UK" + sizes.split("Size")[-1].replace('"',"") + colors + 'HKD,,,,,,,,,,,,,,,,\n'
              else:
                strWrite += 'SKU,[S]Size= ' + sizes.split("Shoe")[-1].replace("Size","") + '; [S]' + item['id1'].extract()[0].replace("\n","").replace("\r","").replace("  ","") + "=" + colors + ',,,,,,' + item ["Product_Code"]+ "UK" + sizes.split("Size")[-1].replace('"',"") + colors + 'HKD,,,,,,,,,,,,,,,,\n'
          else:           
            strWrite += 'SKU,[S]Size= ' + sizes.split("Shoe")[-1].split("Hockey Stick Size ")[-1].replace("Size","UK") + ',,,,,,' + item ["Product_Code"]+"UK"+ sizes.split("Hockey Stick Size ")[-1].split("Size")[-1].replace(" ","") +'HKD,,,,,,,,,,,,,,,,\n'
...
#--BASIC PRODUCT DATA STARTS HERE--
  ...
  #def parse(self,response):
    ...
    #Pricing
    item ["Retail_Price"] =  str(float(re.findall(r'\d+\.?\d+|\d+',sel.xpath("//p[@class='fLeft']/span/text()").extract()[0])[0]) * 97*123/100 + 2000)
    item ["Price"] =  str(float(re.findall(r'\d+\.?\d+|\d+',sel.xpath("//p[@class='fLeft']/span/text()").extract()[0])[0]) * 97*123/100 + 2000)
    sp = response.xpath("//div[@class='price']/p/strong/text()").re(r'\d+')
    if sp:
      item ["Sale_Price"] = str(float(re.findall(r'\d+\.?\d+|\d+',sel.xpath("//div[@class='price']/p/strong/text()").extract()[0])[0]) *  97*123/100 + 2000)
    else:
      item ["Sale_Price"] = str(float(re.findall(r'\d+\.?\d+|\d+',sel.xpath("//span[@class='NowPriceValue']/text()").extract()[0])[0]) * 97*123/100 + 2000)
