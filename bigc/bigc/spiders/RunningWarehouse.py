...
class runningwarehouse(CrawlSpider):
  ...
  def to_csv(self, item):
      ...
      if ("Null") not in item["Brand_Name"]:
        ...
        row += item["Product_Image_File1"], item["Product_Image_File2"]+","+','.join(item["Product_Image_File3"]) +','         
  ...
  #def parse(self, response):
      ...
      #Pricing
      if list1:
        x= float(response.xpath("//span[@class='list']/text()").extract()[0].replace("$","").replace("MSRP: ","").replace("msrp: ",""))
        item ["Retail_Price"] = str((x*65 + 2100)*114.5/100 + x*65*23/100)
        item ["Sale_Price"] = ""        
        item ["Price"] = str((x*65 + 2100)*114.5/100 + x*65*23/100)
        if sp:
          y= float(response.xpath("//span[@class='sale']/text()").extract()[0].replace("$","").replace("Sale: ","").replace("Price: ","").replace("To View Add to Cart","0"))
          z= (100-(((x-y)/x)/4*100))*x/100
          if x*.5<z:
            item ["Sale_Price"] = str((z*65 + 2100)*114.5/100 + z*65*20/100)            
          else:
            item ["Sale_Price"] = ""

      else:
        x= float(response.xpath("//span[@class='price']/text()").extract()[0].replace("$","").replace("Price: ","").replace("MSRP: ","").replace("To View Add to Cart","0"))
        item ["Retail_Price"] = str((x*65 + 2100)*114.5/100 + x*65*23/100)
        item ["Price"] = str((x*65 + 2100)*114.5/100 + x*65*23/100)
        item ["Sale_Price"] = ""
      ...
      if item["Brand_Name"] in ("Asics","Saucony","Mizuno","Brooks","New Balance","ASICS"):
        item ["Sort_Order"] = str(float(-400-(x*20/100)))
      elif item["Brand_Name"] in ("Hoka One One","Salomon","Altra","Under Armour"):
        item ["Sort_Order"] = str(float(-350-(x*20/100)))
      else:
        item ["Sort_Order"] = str(float(-300-(x*20/100)))
